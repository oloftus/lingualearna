define([ "util/commonTypes", "appRoot", "util/util", "util/messageHandlers" ], function() {
    ...
    var noteController = function($scope, noteService, languageNamesService, messageHandlers, formInput) {
        ...
        if ($scope.model.noteId == null) {
            ...
            opeation = Operations.EDIT;
        }
        
        if (operation == Operations.ADD) {
            $scope.model.operationTitle = "Add Note";
        }
        else {
            $scope.model.operationTitle = "Edit note";
        }
        
        $scope.func.addEditNote = function() {
            
            note = new Note($scope.model.foreignLang, $scope.model.foreignNote, $scope.model.localLang, $scope.model.localNote,
                    $scope.model.additionalNotes, $scope.model.sourceUrl, $scope.model.noteId);
            
            if (operation == Operations.ADD) {
                noteService.create(note, function(data) {
                    messageHandlers.clearAllMessages($scope);
                    messageHandlers.addGlobalMessage($scope, "Note saved", MessageSeverity.INFO);
    ...
    linguaApp.controller("noteController", [ "$scope", "noteService", "languageNamesService", "messageHandlers", "formInput", noteController ]);
});
